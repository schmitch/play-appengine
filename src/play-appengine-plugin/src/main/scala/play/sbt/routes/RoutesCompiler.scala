...
import Compiler.{ GeneratedSource, RoutesCompilerTask }
import sbt._
import sbt.Keys._
import com.typesafe.sbt.web.incremental._
import de.envisia.google.sbt.appengine.Versions
...
object RoutesCompiler extends AutoPlugin with RoutesCompilerCompat {
  ...
  def compileRoutes(tasks: Seq[RoutesCompilerTask], generator: RoutesGenerator, generatedDir: File,
    ...
    val ops = tasks.map(task => RoutesCompilerOp(task, generator.id, Versions.PlayVersion))
